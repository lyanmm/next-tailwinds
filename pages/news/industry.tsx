...
export default function Industry() {
  ...
  useEffect(() => {
    ...
    Http.getNewsData(1, 6, 1).then(({data: {result: newsData, pager: {total}}}) => {
  ...
  useEffect(() => {
    if (!initRef.current) {
      Http.getNewsData(page, 2, 1).then(({data: {result: newsData, pager: {total}}}) => {
        setData(total, newsData);
      });
    } else {
    ...
    <Layout header={{y: 100}} footer={setting} seo={seo}>
      ...
      <div className={styles.tabs}>
        <div className={styles.tab}>
          <span
            className={cn(styles.active, 'rounded select-none cursor-pointer text-cblack trans-motion')}>行业动态</span>
          <Link href={'/news/company' + process.env.NEXT_PUBLIC_SUFFIX_HTML}>
            <span
              className={'hover:bg-tab rounded select-none cursor-pointer text-cblack trans-motion'}>公司动态</span>
          </Link>
        </div>
      </div>
      <div className={'mb-24'}>
      ...
      <CustomButton
        ...
        className={cn(btnState.show ? 'block' : 'hidden', 'mx-auto mb-16 mt-16 text-white w-50 h-16 bg-bang text-2xl')}>
